...
def delete_character_api(character_id):
    try:
        ...
        # Знаходимо персонажа
        character = Character.query.filter_by(id_character=character_id, id_user=user_id).first()
        if not character:
        ...
        # Потім видаляємо атаки та нотатки
...
def charlist_api():
    try:
        ...
        # Отримуємо список персонажів, які належать цьому користувачеві
        characters = Character.query.filter_by(id_user=user_id).all()
